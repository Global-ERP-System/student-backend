...
//-------------------------GRADE-INFO--------------------------
router.get('/grade/:roll_no', (req, res) => {
  const roll_no = req.params.roll_no;
  Profile.find({ roll_no: roll_no }, { grades: 1 }, function (err, gr) {
    if (!err) {
      res.json(gr);
    } else {
      res.json(err);
...
router.get('/backlog/:roll_no', (req, res) => {
  const roll_no = req.params.roll_no;
  Profile.find({ roll_no: roll_no }, { grades: 1 }, function (err, gr) {
    if (!err) {
      gr[0].grades.forEach((item) => {
        const newItem = item.gradeInfo.filter(function (g) {
          return g.grade === 'F';
        });
        item.gradeInfo = newItem;
      });
      res.json(gr);
    } else {
      res.json(err);
...
router.patch('/addGrade/:roll_no', function (req, res) {
  const roll_no = req.params.roll_no;
  const grades = req.body;
  Profile.update({ roll_no: roll_no }, { $push: { grades: grades } }, function (
    err
  ) {
    if (!err) {
